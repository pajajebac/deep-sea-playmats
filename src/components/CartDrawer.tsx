    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      ...
      <SheetContent className="w-full sm:max-w-lg flex flex-col h-full bg-card">
        <SheetHeader className="flex-shrink-0">
          <SheetTitle className="font-display tracking-wider text-primary">Your Loot</SheetTitle>
          <SheetDescription>
            {totalItems === 0 ? "Your chest is empty" : `${totalItems} item${totalItems !== 1 ? 's' : ''} in your chest`}
          </SheetDescription>
        </SheetHeader>
          ...
          {items.length === 0 ? (
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-muted-foreground font-body">No treasure here yet...</p>
              </div>
            </div>
          ) : (
            <>
              <div className="flex-1 overflow-y-auto pr-2 min-h-0">
                <div className="space-y-4">
                  {items.map((item) => (
                    <div key={item.variantId} className="flex gap-4 p-2 rounded-md">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="font-display font-semibold text-primary">{item.price.currencyCode} {parseFloat(item.price.amount).toFixed(2)}</p>
                      </div>
              ...
              <div className="flex-shrink-0 space-y-4 pt-4 border-t border-border">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-display text-xl font-bold text-primary">{items[0]?.price.currencyCode || '$'} {totalPrice.toFixed(2)}</span>
                </div>
